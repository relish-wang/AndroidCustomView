 ...
 * @since 20191010
 */
open class ChartDrawable constructor(
    data: ChartData
) : IDrawable<ChartData>(data) {

    /**
     * 更新图表数据📈
     */
    override fun update(data: ChartData) {
        mData = data
        updateSelection()
    }
    ...
    override fun draw(canvas: Canvas, w: Float) {
        ...
        // 画折线图和日期
        drawChart(canvas, w, w * Dimens.CHART_AREA_HEIGHT)
    }
    ...
    private fun drawChart(canvas: Canvas, w: Float, h: Float) {
        ...
        if (abs(highMaxY - lowMinY) >= Dimens.ZERO_BIGGER) { // highMaxY大于lowMinY的话
